...
2.4.2 修正了 iKuuu 全节点测速按钮定位（服务器页右下角悬浮按钮），保留完整标签并读取 Flutter 深层节点，避免零候选；首次读取时请求语义树并短暂重试。已在 iKuuuVPN 0.17.5 实测主页导航、实时测速、自动切换、切换后网络检查及隐藏窗口后的节点读取。增加 `--ikuuu-diagnose` 只读诊断入口。辅助功能自测须通过 macOS 正常启动 App，直接从终端运行可能继承终端的授权身份。

2.4.1 修复了 iKuuu 停在主页时被误判为不兼容的问题：日常检查可读取主页当前节点，恢复操作会通过辅助功能进入服务器页后测速。同时修复中文 Wi-Fi 名称乱码、重复节点标签和相似节点名误匹配。iKuuu 主窗口需保持可供辅助功能读取；关闭窗口或权限失效时会暂停自动切换。
