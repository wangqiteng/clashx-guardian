...
一个低资源后台检查器和原生 macOS 菜单栏状态应用。它只在连接到配置的 Wi-Fi、Clash 控制接口可用且系统代理已开启时进行检查。连续 20 秒无法通过本地 Clash 代理访问 Codex 后端时，它会并发测试候选节点、结合历史可靠性选择节点，并再次验证真实连通性。
...
- 只有连续失败满 20 秒并再次确认失败才测速；默认每批并发测速 3 个、总计最多 8 个候选。
- 历史成功率、连续失败和延迟会参与排序；候选窗口会轮换，不会永远只测试策略组前几个节点。
...
- 订阅节点很多时，每轮最多测试 `MAX_CANDIDATES` 个，但候选窗口会自动轮换。可适当增大数值；并发上限由 `MAX_PARALLEL_TESTS` 控制，建议保持 2～4，避免同时发起过多测速请求。
